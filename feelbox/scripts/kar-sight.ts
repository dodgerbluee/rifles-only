...
if (ironFore && ironFore instanceof THREE.Mesh && ironRear) {
  ...
  const hoodHex = (ironFore.material as THREE.MeshStandardMaterial).color.getHex();
  ...
  check("boxy U sheen differs from the rounded U", (ironFore.material as THREE.MeshStandardMaterial).roughness !== (rearMesh.material as THREE.MeshStandardMaterial).roughness);
  check("boxy and rounded Us are separate meshes", hoodHex === rearHex && ironFore !== ironRear);
